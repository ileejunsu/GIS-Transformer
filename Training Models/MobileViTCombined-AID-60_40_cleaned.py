...
def conv_block(x, filters=16, kernel_size=3, strides=2):
    ...
    return conv_layer(x)
def depth_conv_block(x, filters=16, kernel_size=3, strides=2):
    depth_conv_layer = layers.SeparableConv2D(
        filters, kernel_size, strides=strides, activation=tf.nn.swish, padding="same"
    )
    return depth_conv_layer(x)
...
def mobilevit_block(x, num_blocks, projection_dim, strides=1):
    ...
    local_features = depth_conv_block(x, filters=projection_dim, strides=strides)
...
def conv2d_block_3(input_tensor, n_filters, kernel_size):
    x = layers.DepthwiseConv2D(kernel_size=(3, 3), strides=(2, 2), padding='same',
                        data_format=None, dilation_rate=(1, 1), depth_multiplier=1, activation=None, use_bias=True,
    ...
    x = layers.ReLU()(x)
    print(x.shape)
    y = layers.Conv2D(filters=n_filters, kernel_size=(kernel_size,kernel_size), padding='same')(input_tensor)
    print(y.shape)
    y = layers.BatchNormalization(axis=-1)(y)
    print(y.shape)
    y = layers.ReLU()(y)
    print(y.shape)
    y = layers.DepthwiseConv2D(kernel_size=(3, 3), strides=(2, 2), padding='same',
                        data_format=None, dilation_rate=(1, 1), depth_multiplier=1, activation=None, use_bias=True,
                        depthwise_initializer='glorot_uniform',
                        bias_initializer='zeros', depthwise_regularizer=None,
                        bias_regularizer=None, activity_regularizer=None, depthwise_constraint=None,
                        bias_constraint=None)(y)
    print(y.shape)
    y = layers.Conv2D(filters=n_filters, kernel_size=(kernel_size,kernel_size), padding='same')(y)
    print(y.shape)
    y = layers.BatchNormalization(axis=-1)(y)
    print(y.shape)
    y = layers.ReLU()(y)
    print(y.shape)
    shuffle_net = layers.Concatenate(axis=-1)([x,y])
    return shuffle_net
...
def create_mobilevit(num_classes=30):
    ...
    x = conv2d_block_3(x, n_filters=320, kernel_size=1)
...
def run_experiment(epochs=epochs):
    ...
    parameters_log = CSVLogger('MobileViTCombined_AID_6040_Parameter_log.csv')
...
save_test.tofile("MobileViTCombined_AID_6040_test_labels.csv", sep=",")
save_predicted = np.array(predicted_labels)
save_predicted.tofile("MobileViTCombined_AID_6040_predicted_labels.csv", sep=",")
...
#Save the model
tf.keras.models.save_model(model, 'MobileViTCombined-AID60_40.h5')
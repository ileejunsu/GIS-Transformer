...
def conv2d_block_1(input_tensor, n_filters, kernel_size=3, batchnorm=True):
...
x = conv2d_block_1(x, n_filters=32)
...
def run_experiment():
    ...
    parameters_log = CSVLogger('SWINConvBlock_AID_6040_Parameter_log.csv')
    model.summary()
...
save_test.tofile("SWINConvBlock_AID_6040_test_labels.csv", sep=",")
save_predicted = np.array(predicted_labels)
save_predicted.tofile("SWINConvBlock_AID_6040_predicted_labels.csv", sep=",")
...
plt.show()
#Save the model
tf.keras.models.save_model(swin_model, 'SWINConvBlock-AID60_40.h5')
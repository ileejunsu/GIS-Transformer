...
def conv2d_block_3(input_tensor, n_filters, kernel_size=3, groupnorm=True):
    print(input_tensor.shape)
    # first layer
    x = layers.SeparableConv2D(filters=n_filters, kernel_size=(kernel_size, kernel_size), strides=(1, 1), padding='same',
                        ...
                        pointwise_constraint=None, bias_constraint=None)(input_tensor)
    if groupnorm:
    ...
    x = layers.SeparableConv2D(filters=n_filters, kernel_size=(kernel_size, kernel_size), strides=(1, 1), padding='same',
                        ...
                        bias_initializer='zeros', depthwise_regularizer=None, pointwise_regularizer=None,
                        bias_regularizer=None, activity_regularizer=None, depthwise_constraint=None,
                        pointwise_constraint=None, bias_constraint=None)(x)
    if groupnorm:
        x = layers.GroupNormalization(groups=8, axis=-1)(x)
        x = layers.Activation("relu")(x)
    return x
...
x = conv2d_block_3(x, n_filters=32)
...
def run_experiment():
    ...
    parameters_log = CSVLogger('SWINShuffleNet_AID_6040_Parameter_log.csv')
    model.summary()
...
save_test.tofile("SWINShuffleNet_AID_6040_test_labels.csv", sep=",")
save_predicted = np.array(predicted_labels)
save_predicted.tofile("SWINShuffleNet_AID_6040_predicted_labels.csv", sep=",")
...
#Save the model
tf.keras.models.save_model(swin_model, 'SWINShuffleNet-AID60_40.h5')